...
import java.util.Optional; //imported at 12:44, not sure is correct
...
public class JPAMappingsTest {
	...
	@Resource
	private TestEntityManager entityManager; // will be common practice to use in JPA tests
	...
	public void shouldSaveAndLoadTopic() {
		Topic topic = topicRepo.save(new Topic("topic"));//shortcut of creation (long version for save course test)
	...
	public void shouldSaveAndLoadCourse() {
		Course course = new Course("course name", "description");//these two lines are long version of topic tests, above
	...
	public void shouldEstablishCourseToTopicRelationships() {
		//topic is not the owner so we must create topics first
		Topic java = topicRepo.save(new Topic("Java"));
		Topic ruby = topicRepo.save(new Topic("Ruby"));
		
		Course course = new Course("OO Languages", "descrition", java,  ruby);
		course = courseRepo.save(course);
		long courseId = course.getId();
